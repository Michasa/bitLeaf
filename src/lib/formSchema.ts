...
const FormSchema = z.object({
  ...
  amount: z.preprocess(
    ...
    z
      .number()
      .gte(MIN_REQ_AMOUNT, {
        message: `Minimum request is ${MIN_REQ_AMOUNT} BTC`,
      })
      .lte(MAX_REQ_AMOUNT, {
        message: `Maximum request is ${MAX_REQ_AMOUNT} BTC`,
      })
      .nonnegative({ message: "Request cannot be negative" }),
  ),
  label: z
    .string()
    .max(MAX_LABEL_CHAR_COUNT, {
      message: `Maximum ${MAX_LABEL_CHAR_COUNT} characters allowed as message`,
    })
    .optional(),
  message: z
    .string()
    .max(MAX_MESSAGE_CHAR_COUNT, {
      message: `Maximum ${MAX_MESSAGE_CHAR_COUNT} characters allowed as message`,
    })
    .optional(),
});
